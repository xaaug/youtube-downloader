...
const fmtViews = (n:number) => n>=1e6?`${(n/1e6).toFixed(1)}M views`:n>=1e3?`${(n/1e3).toFixed(0)}K views`:`${n} views`;
const fmtSize  = (b:number|null) => b?`${(b/1024/1024).toFixed(0)}MB`:"";
const genId    = () => (crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).slice(2));
const isYtUrl  = (u:string) => { try{const x=new URL(u);return x.hostname.includes("youtube.com")||x.hostname.includes("youtu.be");}catch{return false;}};

type Fmt = { format_id:string; quality:string; height:number; ext:string; filesize:number|null };
type Meta = { title:string; thumbnail:string; duration:number; uploader:string|null; view_count:number|null; formats:Fmt[]; is_playlist:boolean };
...
export default function YouTubePage({ onAmbient }: Props) {
  ...
  const [dlState,  setDlState]  = useState<"idle"|"active"|"done"|"error">("idle");
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      if (isYtUrl(u)) { setUrl(u); setMeta(null); setErr(""); }
    };
  ...
  const loadMeta = async () => {
    if (!canFetch) return;
    setFetching(true); setMeta(null); setErr(""); setTrimOn(false); setDlState("idle"); setProg({}); onAmbient(null);
    try {
      ...
      setStart(0); setEnd(d.duration || 0);
  ...
  const startDownload = (retryCount = 0) => {
    ...
    const id = genId();
    lastId.current = id;
    setDlState("active"); setProg({ status: "starting", percent: 0 }); setRetries(retryCount);
    ...
    sse.onmessage = e => {
      try {
        ...
        if (d.status === "completed") {
          setDlState("done");
          sse.close();
          addToHistory({ title: meta.title, url, thumbnail: meta.thumbnail, format });
          setTimeout(() => setDlState("idle"), 6000);
        }
        if (d.status === "error" || d.status === "failed") {
          sse.close();
          if (retryCount < MAX_RETRY) {
            setTimeout(() => startDownload(retryCount + 1), 1500 * (retryCount + 1));
          } else {
            setDlState("error");
  ...
  const btnLabel = () => {
    ...
    if (dlState === "error")  return retries > 0 ? `✗ Failed after ${retries} retries` : "✗ Failed — click to retry";
    if (dlState === "active") {
  ...
  return (
    <>
      <div className="pg-head">
        ...
        <h1 className="pg-head__h1">Download <em>anything.</em></h1>
        <p className="pg-head__sub">Video, audio, playlists — trimmed to exactly what you need.</p>
      </div>
        ...
        <>
          ...
          <div className="v-card anim-up">
            <div className="v-card__thumb">
              ...
              {meta.duration > 0 && <span className="v-card__dur">{fmt(meta.duration)}</span>}
            </div>
            <div className="v-card__info">
              <span className="v-card__badge">{meta.is_playlist ? "Playlist" : "Video"}</span>
            ...
            <>
              ...
              <div className="chips anim-up-3">
                {meta.formats.map((f,i) => (
                  <button key={f.quality} className={`chip${quality===f.quality?" on":""}${i===0?" best":""}`} onClick={() => setQuality(f.quality)}>
                    {f.quality}
                    {f.filesize && <span className="size-badge">{fmtSize(f.filesize)}</span>}
                  </button>
                ))}
              </div>
            </>
          ...
          {/* TRIM TOGGLE */}
          {meta.duration > 0 && (
            <button className={`trim-toggle anim-up-4${trimOn?" on":""}`} onClick={() => setTrimOn(v=>!v)}>
          ...
          {trimOn && meta.duration > 0 && (
            <Trimmer duration={meta.duration} startTime={start} endTime={end} onChange={(s,e)=>{setStart(s);setEnd(e);}} />
          )}

          {/* DOWNLOAD BTN */}
          <button
            className={`dl-btn${dlState==="active"?" busy":dlState==="done"?" done":dlState==="error"?" fail":""}`}
            onClick={() => dlState === "error" ? startDownload(0) : dlState === "idle" ? startDownload(0) : undefined}
          ...
          {dlState === "active" && (
            <div className="prog anim-in">
              <div className="prog__row">
                <span>{prog.status || "working"}{retries > 0 ? ` · retry ${retries}` : ""}</span>
